...
def audit(map_yaml,scenario):
    ...
    samples=samples[inside];indices=indices[inside]
    distances,_=cKDTree(points).query(samples)
    in_free=free[indices[:,1],indices[:,0]]
    unexplained=in_free&(distances>.15)
    return {'true_wall_samples':len(samples),'true_wall_coverage_within_15cm':float(np.mean(distances<=.15)),
...
p=argparse.ArgumentParser();p.add_argument('reports');a=p.parse_args();rows=[]
